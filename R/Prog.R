...
message_switch <- function(type, verbose = TRUE, ...) {
    ...
    if (verbose) {
        ...
        "pca_rgb_tensor" = cat(paste(t,
            " Converting PCA Embediing Values to RGB \n")),
        "pcal_rgb_tensor" = cat(paste(t,
            " Converting Loading Values to RGB in PC", args$pc, "\n")),
        "svd_tensor" = cat(paste(t,
            " Running Single Value Decomposition \n")),
        "svd_rgb_tensor" = cat(paste(t,
            " Converting Embeddings to RGB \n")),
        "umap_rgb_tensor" = cat(paste(t,
            " Converting UMAP Embeddings to RGB \n")),
        ...
        "reg" = cat(paste(t,
            " Regularising Image \n")),
        "seg" = cat(paste(t,
            " Segmentation Iteration ", args$seg, "\r")),
        "smooth" = cat(paste(t,
            " Smoothing Image Arrays \n")),
        "ter_pool" = cat(paste(t,
            " Pooling territory ", args$ter, "\r")),
        "eq" = cat(paste(t,
        ...
        "assay_name" = cat(paste(t,
            " No assay names provided - Using default \n",
