...
output of building RGB images from transcrtiptional profiles.
...
this count matrix to compute differential gene expression. Thus, we will
load and pre-process data sets required for downstream analysis.

```{r Vesalius_MethodCompSeurat, eval=TRUE, echo=TRUE, warning= FALSE,message=FALSE}
...
#pdf("seu_log.pdf", width =7, height=6)
#coord_seu
...
```{r Vesalius_MethodCompGiotto, eval = TRUE, echo = TRUE}
library(Giotto)
# Standard libraries - just in case you are a command line fiend
library(utils)
library(stats)
...
giotto <- createGiottoObject(raw_exprs = slideCounts,
                             spatial_locs = slideBeads,
                             instructions = instrs)
...
## normalize
giotto <- normalizeGiotto(gobject = giotto, scalefactor = 10000, verbose = T)
giotto <- addStatistics(gobject = giotto)
giotto <- adjustGiottoMatrix(gobject = giotto, expression_values = c('normalized'),
                             batch_columns = NULL, covariate_columns = c('nr_genes', 'total_expr'),
                             return_gobject = TRUE,
                             update_slot = c('custom'))

giotto <- calculateHVG(gobject = giotto,method = "cov_loess",
                       save_param = list(save_name = 'SSV2_HVG'))

#giotto ## run PCA on expression values (default)
...
giotto <- runUMAP(giotto, dimensions_to_use = 1:30, n_components = 2, n_threads = 4)


giotto <- createNearestNetwork(gobject = giotto, dimensions_to_use = 1:30, k = 15)
## Leiden clustering
giotto <- doLeidenCluster(gobject = giotto, resolution = 0.2, n_iterations = 200,
                                name = 'leiden_0.2')

giotto <- giotto@cell_metadata

#### check what the column names actually are
#locs <- giotto@spatial_locs
locs <- read.table(slideBeads, header=T, sep = ",")
######################

giotto <- cbind(giotto, locs)


giotto_col <- length(unique(giotto$leiden_0.2))
giotto_pal <- colorRampPalette(brewer.pal(8, "Accent"))


coord_giotto <- ggplot(giotto, aes(xcoord,ycoord,col = as.factor(leiden_0.2))) +
            geom_point(size = 0.25, alpha = 0.65) +
            theme_void() +
            scale_color_manual(values = giotto_pal(giotto_col)[sample(1:giotto_col,giotto_col)]) +
            theme(legend.text = element_text(size = 12),
                  legend.position = "right",
                  plot.title = element_text(size=15)) +
            guides(colour = guide_legend(override.aes = list(size=5)))+
            labs(colour = "Cluster nr.", title = "Giotto - Clusters")

#pdf("giotto_log.pdf", width =7, height=6)
coord_giotto
...
```{r Vesalius_MethodCompBayesSpace, eval = FALSE, echo= TRUE}
library(BayesSpace)
...
.find_neighbors <- function(sce, platform) {
    # we are going to use switch instead
    # it will be easier for our purpose
...
## this is essentially the bayes space original function
...
bayesData$spatial.cluster <- as.factor(bayesData$spatial.cluster)
...
#pdf("bayes_log.pdf", width =7, height=6)
coord_bayes
...
```{r vesalius_methodCompPlot, eval = F, echo = T}
pdf("vesalius_methodComp.pdf",width= 15, height = 5.25)
(coord_bayes + coord_seu + coord_giotto) + plot_layout(width =c(1,1,1))
dev.off()
...
```{r st_data_comp, eval = TRUE , echo =TRUE}
...
#----------------------------------------------------------------------------#
# We can export the pretty pictures
#----------------------------------------------------------------------------#
...
```{r layered_subcortex, eval =TRUE, echo =T,fig.width=9, fig.height =7}
