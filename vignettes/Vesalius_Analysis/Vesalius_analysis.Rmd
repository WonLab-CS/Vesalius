...
imageBrain <- smoothArray(imageBrain,
                          iter = 20 ,
                          method = c("iso","box"),
                          sigma = 1,
                          box = 15)
pdf("smooth_brain.pdf")
imagePlot(imageBrain,as.cimg=F)
dev.off()
...
imageEmbryo <- smoothArray(imageEmbryo,
                           iter = 20,
                           method = c("iso","box"),
                           sigma =1,
                           box = 10)
pdf("smooth_embryo.pdf")
imagePlot(imageEmbryo,as.cimg =F)
dev.off()
...
This does not affect in anyway the downstream analysis. It only serve as a way of handling the data internally. We ran Vesalius on all 3 seqFISH embryo data sets available.
...
PCtag <- c("PC1 - PC3", "PC4 - PC6","PC7 - PC9","PC10 - PC12","PC13 - PC15")
...
for(i in seq_along(slideTag)){
  ...
  umaps <- buildImageArray(umaps,
                          ...
                          resolution = 40,
                          cores=10)
  plots[[1]] <- imagePlot(umaps, as.cimg =F, cex = 12) + 
      theme_void()+
      labs(title = "UMAP - Image")
  lambda <- ifelse(i==2,10,5)
  umaps <- regulariseImage(umaps,
                              lambda = lambda,
                              niter = 50,
                              normalise=T)
  umaps <- iterativeSegmentation.array(umaps, colDepth =11,
                                       smoothIter = 20,
                                       method = c("iso","box"),
                                       sigma=1,box = 15,
                                       useCenter = T)
  umaps <- isolateTerritories.array(umaps,captureRadius = 0.008,
                                        minBar = 50)
  
  plots[[2]] <- territoryPlot(umaps, randomise = TRUE,cex =12 , cex.pt=0.25) +
                  theme_void()+
                  theme(legend.text = element_text(size = 12),
                  legend.title = element_text(size=12),
                  plot.title = element_text(size =12),
                  legend.position = "none")+
                  labs(title = "UMAP - Territory")
  #----------------------------------------------------------------------------#
  ...
  for(j in seq_along(unique(pcs$slice))){
    ...
    plots[[counter]] <- imagePlot(tmp,as.cimg =F, cex=12) + 
      theme_void()+
      labs(title = paste(PCtag[j], "- Image"))
    counter <- counter + 1
    
    tmp <- regulariseImage(tmp,
                              lambda = lambda,
                              niter = 50,
                              normalise=T)
    tmp <- iterativeSegmentation.array(tmp, colDepth =11,
                                       smoothIter = 20,
                                       method = c("iso","box"),
                                       sigma=1,box = 15,
                                       useCenter = T)
    tmp <- isolateTerritories.array(tmp)
   
    plots[[counter]] <- territoryPlot(tmp, randomise = TRUE,cex =15 , cex.pt=0.25) +
                  theme_void()+
                  theme(legend.text = element_text(size = 12),
                  legend.title = element_text(size=12),
                  plot.title = element_text(size =12),
                  legend.position = "none")+
                  labs(title = paste(PCtag[j], "- Territories"))
    counter <- counter + 1
  }
  #----------------------------------------------------------------------------#
  # Plot the outputs
  ...
  pdf(fileOut, width = 10, height = 7.5)
