...
*NOTE that the path to files have not been changed - change path to files
accordingly*
...
brain and embryo (E12.5). We also used SeqScope data available [here]()
...
then rasterised to include all possible pixel location and assigned its colour
code. We will only use of the arrays as an example. The analysis of the second
array will be shown below.
...
The resolution argument changes to resolution of the output image. Smaller Images
decrease run time. It should be noted that while the image resolution may be
decreased, all beads are retained.
...
Vesalius also offers the possibility to equalise color histogram to increase
...
smooth independantly, please use the `smoothArray`function.*
...
The figure produce here is *Figure 2A* in the manuscript.
...
epiComp <- FindMarkers(medCluster,ident.1 =1,ident.2=6)
...
coord_med <- ggplot(med, aes(x,y,col = as.factor(seurat_clusters))) +
            ...
            labs(colour = " ", title = "",
                x = "X coordinates", y = "Y coordinates")
```
Finally, we can visualize cell clustering in isolated territories
...
in hippocampus still apply here.
...
write.table(by_cell, file = "vesalius_terComp_CortexVSThalamus.csv")
```
...
# UMAP vs PCA
In the section, we will aim to demonstrate the differences between using UMAP or
...
that more of the data can be included into the final images. While this does
...
images. Obvisouly, in this case there is less information included in the final
...
that this approach can contribute to really capitalising on the information
...
manuscript. Figures related to benchmarking and simulations are produced with
the script found (here)[]



All ISH images are taken from the Allen Brain Atlas.
