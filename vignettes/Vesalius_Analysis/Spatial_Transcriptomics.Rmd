...
This section does not contain any code related to the vesalius package but demonstrate the approach we took
for gene functional analysis.

```{r loading_func_lib, eval = TRUE , echo = TRUE}
library(DOSE)
library(pathview)
...
Since we already have the data frames we can use these directly. Note that DEG data frame were saved as `.csv`.


```{r gprofiler2, eval = TRUE, echo = TRUE}
top_brain <- split(deg_brain, deg_brain$seed)
brain_gost <- lapply(top_brain, function(x) {
    over <- gost(x$genes,
      organism = "mmusculus",
      significant = TRUE)
    under <- gost(x$genes,
    ...
    return(list("over" = over, "under" = under))
})

top_embryo <- split(deg_embryo, deg_embryo$seed)
embryo_gost <- lapply(top_embryo, function(x) {
    over <- gost(x$genes,
      organism = "mmusculus",
      significant = TRUE)
    under <- gost(x$genes,
