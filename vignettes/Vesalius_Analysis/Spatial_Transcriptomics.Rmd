...
by combining neighboring spatial indices if they fall within the local area. Reducing the image
resolution ensure faster run times. 

In addition to merging spatial indices, we adjust the count matrix by taking the average count values
for all spatial indices that have been merged together. Note that the names become a combination of 
the original spatial_indices.
...
- **To only generate tiles** see[generate_tiles]()
- **To run multiple embeddings**, see [handling multiple embeddings or counts]()
...
```{r plot_grey_raw_brain, eval = TRUE, echo = TRUE, fig.width = 20, fir.height =  20}
print(ggarrange(plotlist = pca_embedding_brain), ncol = 5)
```

For the mouse embryo:
```{r plot_grey_raw_embryo, eval = TRUE, echo = TRUE, fig.width = 20, fir.height =  20}
...
```{r plot_grey_pro_brain, eval = TRUE, echo = TRUE, fig.width = 20, fir.height =  20}
print(ggarrange(plotlist = pca_embedding_brain), ncol = 5)
```

For the mouse embryo:
```{r plot_grey_pro_embryo, eval = TRUE, echo = TRUE, fig.width = 20, fir.height =  20}
print(ggarrange(plotlist = pca_embedding_embryo), ncol = 5)
```
...
vesalius_brain <- segment_image(vesalius_brain,
    ...
    method = "kmeans",
    col_resolution = 25)
vesalius_brain <- isolate_territories(vesalius_brain)

vesalius_embryo <- segment_image(vesalius_embryo,
    dimensions = seq(1, 30),
    method = "kmeans",
    col_resolution = 13)
vesalius_embryo <- isolate_territories(vesalius_embryo)
```
...
Conceptually, this plays a similar role as "granularity" resolution in louvain or leiden clustering. 
...
`col_resolution` and the *capture radius* defined in [isolate_territories](). In short, this 
parameters defines the maximum distance between color segments for them to be pooled
