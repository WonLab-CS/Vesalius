...
vesalius <- smooth_image(vesalius, verbose = FALSE)
...
Vesalius will attempts to segment the images intocolor segments. Note that you can run the 
smoothing directly in the `segment_image` function. Vesalius use a kmeans clustering approach 
to segment grey scale images. Note that the segmentation here is applied to 
the whole stack and not individually. 
```{r segment_image, eval = TRUE, echo = TRUE}
vesalius <- segment_image(vesalius, col_depth = 5, verbose = FALSE)
```
